...
namespace Ship.Ses.Extractor.Application.Services.Transformers
{
    ...
    public class ConditionTransformer : IResourceTransformer<JsonObject>
    {
        private readonly ILogger<ConditionTransformer> _logger;

        public ConditionTransformer(ILogger<ConditionTransformer> logger)
        {
            _logger = logger;
        }
        ...
        {
            var enumFields = new[] { "status", "use", "clinicalStatus", "verificationStatus" };
