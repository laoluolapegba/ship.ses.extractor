...
namespace Ship.Ses.Extractor.Application.Services.Transformers
{
    ...
    public class ObservationTransformer : IResourceTransformer<JsonObject>
    {
        private readonly ILogger<ObservationTransformer> _logger;

        public ObservationTransformer(ILogger<ObservationTransformer> logger)
        {
            _logger = logger;
        }
                ...
                if (!string.IsNullOrWhiteSpace(field.Template))
                {
                    switch (field.Template)
                    {
                        ...
                        case "reference":
                            ...
                            break;
                        case "quantity":
                            TemplateBuilders.ApplyQuantity(fhir, field, row, _logger);
                            break;
                        default:
